...
export default function PropertyForm(){const {id,offeredByContactId}=useLocalSearchParams<{id?:string;offeredByContactId?:string}>();const [propertyId]=useState(()=>id??createId('property'));
  const {language,isRTL,t}=useI18n();const L=(ar:string,en:string)=>language==='ar'?ar:en;
  ...
  const activityOptions=language==='ar'?ACTIVITY_OPTIONS:[
    {value:'company_headquarters' as const,label:'Company headquarters'},{value:'educational_institute' as const,label:'Educational institute'},
    {value:'health_institute' as const,label:'Health institute'},{value:'law_office' as const,label:'Law office'},{value:'other' as const,label:'Other activity'}];
  ...
  const launchGoogleMaps=async()=>{const query=encodeURIComponent(`${form.area||'الكويت'}, الكويت`);
    try{await Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${query}`)}catch{Alert.alert(L('تعذر فتح Google Maps','Could not open Google Maps'),L('يمكنك لصق رابط الموقع يدويًا.','You can paste the location link manually.'))}};
  const useCurrentLocation=async()=>{const permission=await Location.requestForegroundPermissionsAsync();
    if(permission.status!=='granted'){Alert.alert(L('الصلاحية مطلوبة','Permission required'),L('اسمح بالموقع أو الصق رابط Google Maps.','Allow location access or paste a Google Maps link.'));return}
    const position=await Location.getCurrentPositionAsync({accuracy:Location.Accuracy.Balanced});
    setLocationDraft(googleMapsUrl(position.coords.latitude,position.coords.longitude));
  };
  const saveLocation=()=>{const url=locationDraft.trim();if(!url){Alert.alert(L('اختر الموقع','Choose location'),L('افتح Google Maps والصق رابط النقطة أو استخدم موقعك الحالي.','Open Google Maps and paste the point link, or use your current location.'));return}
  ...
  const save=async()=>{const rent=Number(form.monthlyRent);if(!form.area.trim()||!Number.isFinite(rent)||rent<=0){Alert.alert(L('بيانات غير مكتملة','Missing details'),L('اختر المنطقة واكتب الإيجار الشهري.','Choose an area and enter the monthly rent.'));return}
    const now=new Date().toISOString();await propertiesRepository.upsert({id:propertyId,
      title:form.title.trim()||(language==='ar'?`${typeNames[form.type]} في ${form.area.trim()}`:`${typeNames[form.type]} in ${form.area.trim()}`),type:form.type,area:form.area.trim(),monthlyRent:rent,
  ...
  return <SafeAreaView style={styles.page} edges={['top']}><AppHeader title={id?L('تعديل العقار','Edit property'):L('إضافة عقار للإيجار','Add rental property')}/>
    <KeyboardAwareScrollViewCompat contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <Text style={[styles.label,{textAlign:isRTL?'right':'left'}]}>{t('type')}</Text><View style={[styles.chips,{flexDirection:isRTL?'row-reverse':'row'}]}>{typeValues.map(value=><Pressable key={value}
        onPress={()=>setForm({...form,type:value})} style={[styles.chip,form.type===value&&styles.active]}><Text style={form.type===value&&styles.activeText}>{typeNames[value]}</Text></Pressable>)}</View>
      <FormField label={L('اسم مختصر للعقار (اختياري)','Short property name (optional)')} placeholder={L('مثال: شقة السالمية 12','Example: Salmiya Apt 12')} value={form.title} onChangeText={title=>setForm({...form,title})}/>
      <Text style={[styles.helper,{textAlign:isRTL?'right':'left'}]}>{L('هذا اسم داخلي يساعدك على تمييز الوحدة بسرعة، وليس عنوان المنطقة.','This is an internal name to identify the unit quickly, not its area address.')}</Text>
      <AreaPicker value={form.area} onChange={area=>setForm({...form,area})}/>
      <FormField label={`${t('rent')} *`} placeholder={L('بالدينار الكويتي','Kuwaiti dinar')} value={form.monthlyRent} keyboardType="numeric" onChangeText={monthlyRent=>setForm({...form,monthlyRent})}/>
      ...
      {commercial&&<>
        <NumberPicker label={L('عدد الأرقام الآلية','PACI number count')} value={form.paciNumberCount} onChange={paciNumberCount=>setForm({...form,paciNumberCount})}/>
        <ChoicePicker label={L('نوع النشاط','Activity type')} value={form.activityType} placeholder={L('اختر نوع النشاط','Choose activity type')} options={activityOptions}
          onChange={activityType=>setForm({...form,activityType})}/>
      </>}
      <FormField label={L('المساحة م²','Area m²')} value={form.sizeSqm} keyboardType="numeric" onChangeText={sizeSqm=>setForm({...form,sizeSqm})}/>
      <FormField label={t('description')} value={form.description} multiline onChangeText={description=>setForm({...form,description})}/>
      <Text style={[styles.label,{textAlign:isRTL?'right':'left'}]}>{L('التأثيث','Furnishing')}</Text><View style={[styles.chips,{flexDirection:isRTL?'row-reverse':'row'}]}>{([['furnished',L('مفروش','Furnished')],['semi_furnished',L('نصف مفروش','Semi-furnished')],['unfurnished',L('غير مفروش','Unfurnished')]] as const).map(([value,label])=><Pressable key={value}
      ...
      <Text style={[styles.label,{textAlign:isRTL?'right':'left'}]}>{L('حالة العقار','Property status')}</Text><View style={[styles.chips,{flexDirection:isRTL?'row-reverse':'row'}]}>{([['available',t('available')],['rented',t('rented')],['paused',t('paused')]] as const).map(([value,label])=><Pressable key={value}
      ...
      <FormField label={`${t('paci')} (${L('اختياري','optional')})`} value={form.paci} keyboardType="numeric" onChangeText={paci=>setForm({...form,paci})}/>
      <PrimaryButton title={form.mapUrl?L('✓ تم اختيار الموقع — اضغط للتعديل','✓ Location selected — tap to edit'):L('📍 اختر الموقع على Google Maps','📍 Choose location on Google Maps')} onPress={openLocation}/>
      <PrimaryButton title={`${L('إضافة صور أو فيديو','Add photos or video')} (${pendingMedia.length})`} onPress={pick}/><PrimaryButton title={L('حفظ العقار','Save property')} onPress={save}/>
    ...
    <Modal visible={locationOpen} animationType="slide" onRequestClose={()=>setLocationOpen(false)}>
      <SafeAreaView style={styles.locationPage}>
        <View style={[styles.locationHeader,{flexDirection:isRTL?'row':'row-reverse'}]}><Pressable onPress={()=>setLocationOpen(false)}><Text style={styles.cancel}>{t('cancel')}</Text></Pressable><Text style={styles.locationTitle}>{L('اختيار موقع العقار','Choose property location')}</Text></View>
        <View style={styles.locationContent}><Text style={[styles.locationHelp,{textAlign:isRTL?'right':'left'}]}>{L('افتح Google Maps وحدد العقار، ثم اختر مشاركة ← نسخ الرابط، وارجع للصقه هنا.','Open Google Maps, select the property, choose Share → Copy link, then return and paste it here.')}</Text>
          <PrimaryButton title={L('فتح Google Maps','Open Google Maps')} onPress={launchGoogleMaps}/>
          <FormField label={t('map')} placeholder={L('الصق رابط Google Maps هنا','Paste Google Maps link here')} value={locationDraft} onChangeText={setLocationDraft} autoCapitalize="none"/>
          <Text style={styles.or}>{L('أو','or')}</Text><PrimaryButton title={L('استخدام موقعي الحالي','Use my current location')} onPress={useCurrentLocation}/>
          <PrimaryButton title={L('حفظ الموقع','Save location')} onPress={saveLocation} color={colors.green}/></View>
      </SafeAreaView>
    </Modal>
  </SafeAreaView>;
}
