...
export default function ContactsScreen(){const {t,isRTL}=useI18n();const [items,setItems]=useState<Contact[]>([]);const [query,setQuery]=useState('');
  const load=useCallback(()=>{contactsRepository.list(query).then(setItems)},[query]);useFocusEffect(useCallback(()=>{load()},[load]));
  return <SafeAreaView style={styles.page} edges={['top']}><AppHeader title={t('contacts')} query={query} onQueryChange={setQuery}/>
    <View style={styles.actions}><PrimaryButton title={t('addContact')} onPress={()=>router.push('/contact-form')}/>
