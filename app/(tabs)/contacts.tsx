...
export default function ContactsScreen(){const {t,isRTL}=useI18n();const [items,setItems]=useState<Contact[]>([]);
  const labels:Record<ContactRole,string>={tenant:t('tenant'),owner:t('owner'),broker:t('broker'),real_estate_company:t('company'),building_guard:t('guard')};
  ...
  return <SafeAreaView style={styles.page} edges={['top']}><AppHeader title={t('contacts')}/>
    ...
    <FlatList data={items} keyExtractor={x=>x.id} contentContainerStyle={styles.list}
      ...
      renderItem={({item})=><Pressable onPress={()=>router.push({pathname:'/contact-detail',params:{id:item.id}})} style={[styles.card,{flexDirection:isRTL?'row-reverse':'row'}]}>
        <View style={{flex:1}}><Text style={[styles.name,{textAlign:isRTL?'right':'left'}]}>{item.name}</Text>
          <Text style={[styles.phone,{textAlign:isRTL?'right':'left'}]}>{item.phone}</Text>{item.notes?<Text numberOfLines={1} style={[styles.note,{textAlign:isRTL?'right':'left'}]}>{item.notes}</Text>:null}</View>
...
const styles=StyleSheet.create({page:{flex:1,backgroundColor:colors.background},actions:{padding:spacing.md,gap:spacing.sm},list:{padding:spacing.md,gap:spacing.sm,
  ...
  name:{fontSize:18,fontWeight:'700',color:colors.text,writingDirection:'auto'},phone:{fontSize:15,color:colors.muted,marginTop:4,writingDirection:'ltr'},note:{fontSize:12,color:colors.muted,marginTop:3,writingDirection:'auto'},badge:{color:colors.red,fontWeight:'700',maxWidth:110,textAlign:'center'},
