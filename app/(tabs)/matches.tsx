...
export default function MatchesScreen(){const {t}=useI18n();const [requirements,setRequirements]=useState<Requirement[]>([]);const [properties,setProperties]=useState<Property[]>([]);const [contacts,setContacts]=useState<Contact[]>([]);
  ...
  const rows=useMemo<Row[]>(()=>findMatches(requirements,properties).flatMap(match=>{const property=properties.find(item=>item.id===match.propertyId);const requirement=requirements.find(item=>item.id===match.requirementId);if(!property||!requirement)return[];return[{match,property,requirement,contactName:contacts.find(item=>item.id===requirement.contactId)?.name??'عميل'}]}),[contacts,properties,requirements]);
