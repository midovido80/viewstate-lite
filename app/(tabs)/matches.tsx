...
export default function MatchesScreen(){const {t}=useI18n();const [rows,setRows]=useState<Row[]>([]);useFocusEffect(useCallback(()=>{(async()=>{const [requirements,properties,contacts]=await Promise.all([
    requirementsRepository.listActive(),propertiesRepository.list(),contactsRepository.list()]);const matches=findMatches(requirements,properties);setRows(matches.flatMap(match=>{const property=properties.find(x=>x.id===match.propertyId);
    const requirement=requirements.find(x=>x.id===match.requirementId);if(!property||!requirement)return[];return[{match,property,requirement,contactName:contacts.find(x=>x.id===requirement.contactId)?.name??'عميل'}]}})})()},[]));
