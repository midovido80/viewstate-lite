...
export default function ContactAddMethod(){const {t,isRTL}=useI18n();return <SafeAreaView style={styles.page} edges={['top']}>
  ...
  <View style={styles.content}>
    <Text style={[styles.question,{textAlign:isRTL?'right':'left'}]}>{t('addMethodQuestion')}</Text>
    <Text style={[styles.helper,{textAlign:isRTL?'right':'left'}]}>{t('addMethodHelper')}</Text>
    ...
    <Pressable onPress={()=>router.push('/contact-import')} style={[styles.card,{flexDirection:isRTL?'row-reverse':'row'}]}>
      ...
      <View style={styles.copy}><Text style={[styles.title,{textAlign:isRTL?'right':'left'}]}>{t('phoneImport')}</Text><Text style={[styles.description,{textAlign:isRTL?'right':'left'}]}>{t('phoneImportHint')}</Text></View>
      <Ionicons name={isRTL?'chevron-back':'chevron-forward'} size={24} color={colors.muted}/>
    </Pressable>
  </View>
</SafeAreaView>}
