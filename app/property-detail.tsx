...
export default function PropertyDetail(){const {id}=useLocalSearchParams<{id:string}>();const [property,setProperty]=useState<Property|null>(null);const [media,setMedia]=useState<PropertyMedia[]>([]);
  ...
  return <SafeAreaView style={styles.page} edges={['top']}><AppHeader title="تفاصيل العقار"/><ScrollView contentContainerStyle={styles.content}><Text style={styles.title}>{property.title}</Text><Text style={styles.price}>{property.monthlyRent} د.ك شهريًا</Text>
    ...
    {property.paciNumberCount!==null&&<Text style={styles.line}>🔢 {property.paciNumberCount} أرقام آلية</Text>}{property.activityType&&<Text style={styles.line}>💼 {activityLabel(property.activityType)}</Text>}
