  ...
  return <View style={styles.wrap}>
    ...
    <Pressable onPress={()=>setOpen(true)} style={[styles.selector,{flexDirection:isRTL?'row':'row-reverse'}]}>
      <Ionicons name="chevron-down" size={20} color={colors.blue}/><Text style={[styles.value,!value.length&&styles.placeholder]}>
        {value.length?`${value.join('، ')}`:t('chooseAreas')}</Text><Ionicons name="location-outline" size={22} color={colors.red}/>
    </Pressable>
    ...
    <Modal visible={open} animationType="none" hardwareAccelerated onRequestClose={close}>
      <SafeAreaView style={styles.page}>
        <View style={styles.header}><Pressable onPress={close} hitSlop={12}><Text style={styles.done}>{t('doneCount',{count:value.length})}</Text></Pressable>
          <Text style={styles.title}>{t('chooseAreasTitle')}</Text></View>
        <View style={styles.search}><Ionicons name="search" size={21} color={colors.muted}/><TextInput autoFocus value={query} onChangeText={setQuery}
          placeholder={t('typeFirstLetters')} placeholderTextColor={colors.muted} selectionColor={colors.blue} cursorColor={colors.blue} style={[styles.searchInput,{textAlign:isRTL?'right':'left'}]}/></View>
        <SectionList sections={sections} keyExtractor={item=>item} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" automaticallyAdjustKeyboardInsets contentContainerStyle={styles.list}
...
const styles=StyleSheet.create({wrap:{gap:spacing.xs},label:{fontSize:15,fontWeight:'600',color:colors.text,textAlign:'right'},selector:{minHeight:58,
  ...
  value:{flex:1,flexShrink:1,fontSize:16,lineHeight:23,color:colors.text,textAlign:'right',fontWeight:'600'},placeholder:{color:colors.muted,fontWeight:'400'},chips:{flexDirection:'row-reverse',flexWrap:'wrap',gap:spacing.xs},
  ...
  selected:{backgroundColor:'#EEF5FF'},rowText:{flex:1,flexShrink:1,fontSize:16,lineHeight:23,textAlign:'right',color:colors.text,paddingVertical:10,paddingHorizontal:spacing.sm},empty:{textAlign:'center',color:colors.muted,marginTop:60,fontSize:17}});
