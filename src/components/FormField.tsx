...
export const FormField=forwardRef<TextInput,TextInputProps&{label:string}>(function FormField({label,...props},ref) {const {isRTL}=useI18n();return <View style={styles.wrap}>
  <Text style={[styles.label,{textAlign:isRTL?'right':'left'}]}>{label}</Text>
  <TextInput ref={ref} {...props} placeholderTextColor={colors.muted} selectionColor={colors.blue} cursorColor={colors.blue}
