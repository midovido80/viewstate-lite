    ...
    <Modal visible={open} transparent animationType="fade" onRequestClose={()=>setOpen(false)}>
      <Pressable style={styles.backdrop} onPress={()=>setOpen(false)}>
        <SafeAreaView style={styles.sheet}>
          <Text style={[styles.title,{textAlign:isRTL?'right':'left'}]}>{label}</Text>
          {options.map(option=><Pressable key={option.value} onPress={()=>{onChange(option.value);setOpen(false)}} style={[styles.row,{flexDirection:isRTL?'row':'row-reverse'}]}>
            ...
            <Text style={[styles.rowText,{textAlign:isRTL?'right':'left'}]}>{option.label}</Text>
          </Pressable>)}
        </SafeAreaView>
      </Pressable>
    </Modal>
