...
export function useDraftAutosave<T>({enabled,key,value,save,delay=900}:{
  ...
  const enqueue=(expectedGeneration=generation.current)=>{if(!enabled||expectedGeneration!==generation.current)return chain.current;
  ...
  const cancel=()=>{generation.current++;clearTimer();return chain.current.catch(()=>{})};
