...
export async function createAndShareBackup():Promise<void>{const json=await exportSnapshot();const uri=`${FileSystem.cacheDirectory}viewstate-lite-backup-${new Date().toISOString().slice(0,10)}.json`;
  ...
  await Sharing.shareAsync(uri,{mimeType:'application/json',dialogTitle:'حفظ نسخة ViewState Lite الاحتياطية'});}
