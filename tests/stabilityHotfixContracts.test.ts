...
test('V0.5.2 keeps database and backup contracts frozen',()=>{
  const pkg=JSON.parse(readFileSync('package.json','utf8'));const app=JSON.parse(readFileSync('app.json','utf8'));
  assert.equal(pkg.version,'0.5.2');assert.equal(app.expo.version,'0.5.2');assert.equal(app.expo.android.versionCode,10);
  assert.equal(DATABASE_SCHEMA_VERSION,5);assert.equal(BACKUP_FORMAT_VERSION,2);
  assert.match(readFileSync('app/(tabs)/more.tsx','utf8'),/Stability Hotfix · V0\.5\.2/);
});
...
test('property save is guarded, keyboard-safe, draft-safe, and preserves edit creation time',()=>{
  ...
  assert.match(source,/setTimeout\(\(\)=>\{void draftsRepository\.save\(key,form\)\},350\)/);
});
...
test('WhatsApp and WhatsApp Business are explicit Android targets',()=>{
  ...
  assert.match(sender,/package=\$\{PACKAGES\[target\]\}/);assert.match(chooser,/choose\('whatsapp'\)/);assert.match(chooser,/choose\('whatsapp_business'\)/);
