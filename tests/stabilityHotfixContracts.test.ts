...
test('V0.5.6 keeps database and backup contracts frozen',()=>{
  const pkg=JSON.parse(readFileSync('package.json','utf8'));const app=JSON.parse(readFileSync('app.json','utf8'));
  assert.equal(pkg.version,'0.5.6');assert.equal(app.expo.version,'0.5.6');assert.equal(app.expo.android.versionCode,14);
  assert.equal(DATABASE_SCHEMA_VERSION,5);assert.equal(BACKUP_FORMAT_VERSION,2);
  assert.match(readFileSync('app/(tabs)/more.tsx','utf8'),/App Icon Update · V0\.5\.6/);
});
