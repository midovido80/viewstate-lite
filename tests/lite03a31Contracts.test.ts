...
test('LITE-03A3.1 advances app identity without changing persisted contracts',()=>{
  const pkg=JSON.parse(readFileSync('package.json','utf8'));const app=JSON.parse(readFileSync('app.json','utf8'));
  assert.equal(pkg.version,'0.5.1');assert.equal(app.expo.version,'0.5.1');assert.equal(app.expo.android.versionCode,9);assert.equal(DATABASE_SCHEMA_VERSION,5);assert.equal(BACKUP_FORMAT_VERSION,2);
});
