...
test('stability hotfix advances app identity without changing persisted contracts',()=>{
  ...
  assert.equal(pkg.version,'0.5.5');assert.equal(app.expo.version,'0.5.5');assert.equal(app.expo.android.versionCode,13);assert.equal(DATABASE_SCHEMA_VERSION,5);assert.equal(BACKUP_FORMAT_VERSION,2);
});
