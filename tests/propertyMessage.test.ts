...
test('shares block number when available and omits it when absent',()=>{assert.doesNotMatch(createPropertyMessage(property),/رقم القطعة/);assert.match(createPropertyMessage({...property,blockNumber:12}),/رقم القطعة: 12/);assert.match(createPropertyMessage({...property,blockNumber:7},{language:'en'}),/Block number: 7/)});
